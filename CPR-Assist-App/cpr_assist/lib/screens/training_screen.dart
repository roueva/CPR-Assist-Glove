...
class _TrainingScreenState extends ConsumerState<TrainingScreen> with AutomaticKeepAliveClientMixin {
  ...
  void didChangeDependencies() {
    ...
    bleConnection.adapterStateStream.listen((state) {
      if (mounted) {
        setState(() {
          connectionStatus = (state == BluetoothAdapterState.on && bleConnection.isConnected)
              ? "Connected"
              : "Bluetooth OFF";
        });
  ...
  bool _bluetoothPromptShown = false;

  void _showBluetoothPrompt() async {
    if (_bluetoothPromptShown) return;
    _bluetoothPromptShown = true;

    final bleConnection = ref.read(bleConnectionProvider);
    bool bluetoothEnabled = await bleConnection.enableBluetooth(prompt: true);
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!_isDisposed && mounted) {
        setState(() {
          ...
          userTemperature = data['userTemperature'] ?? 0.0;
  ...
  Future<void> _saveSessionData() async {
    ...
    try {
      final sessionData = {
        ...
        'user_temperature_rate': userTemperature ?? 0.0,
